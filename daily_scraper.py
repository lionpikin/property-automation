import requests
import csv
import json
from datetime import datetime

# PUT YOUR APIFY TOKEN HERE
APIFY_TOKEN = "apify_api_l2iSfxruKZ9Hht4WlU1vHMtce0lWfe35g5Lp"

# --- Step 1: Get today's date for the filename ---
today = datetime.now().strftime("%Y-%m-%d")
filename = f"today_sellers_{today}.csv"

# --- Step 2: Scrape PropertyPro ---
print(f"Scraping sellers for {today}...")
pp_actor = "gio21~propertypro-ng-scraper"
pp_url = f"https://api.apify.com/v2/acts/{pp_actor}/run-sync-get-dataset-items?token={APIFY_TOKEN}"
pp_payload = {
    "startUrls": ["https://propertypro.ng/property-for-sale/house"],
    "maxItems": 30
}

all_sellers = []

try:
    response = requests.post(pp_url, json=pp_payload, timeout=180)
    data = response.json()
    
    if isinstance(data, list):
        print(f"Success! Found {len(data)} sellers from PropertyPro.")
        for item in data:
            phone = "N/A"
            link = item.get('url', 'N/A')
            if "phone=" in link:
                try:
                    phone = link.split('phone=')[1].split('&')[0]
                except:
                    phone = "N/A"
                    
            all_sellers.append({
                "source": "propertypro",
                "type": "listing",
                "name": "Agent",
                "phone": phone,
                "details": item.get('title', 'N/A'),
                "link": link,
                "date": today
            })
    else:
        print(f"PropertyPro returned an error: {data}")
        
except Exception as e:
    print(f"PropertyPro Error: {e}")

# --- Step 3: Scrape Nigeria Property Centre (NPC) ---
print("Scraping NPC...")
npc_actor = "crawlerbros~nigeria-property-centre-scraper"
npc_url = f"https://api.apify.com/v2/acts/{npc_actor}/run-sync-get-dataset-items?token={APIFY_TOKEN}"
npc_payload = {
    "mode": "search",
    "listingType": "for-sale",
    "propertyType": "houses",
    "state": "any",
    "maxItems": 30
}

try:
    response = requests.post(npc_url, json=npc_payload, timeout=180)
    data = response.json()
    
    if isinstance(data, list):
        print(f"Success! Found {len(data)} sellers from NPC.")
        for item in data:
            all_sellers.append({
                "source": "npc",
                "type": "listing",
                "name": item.get('agentName', 'N/A'),
                "phone": item.get('phone', 'N/A'),
                "details": item.get('title', 'N/A'),
                "link": item.get('listingUrl') or item.get('url', 'N/A'),
                "date": today
            })
    else:
        print(f"NPC returned an error: {data}")
        
except Exception as e:
    print(f"NPC Error: {e}")

# --- Step 4: Save the file ---
if all_sellers:
    with open(filename, 'w', newline='', encoding='utf-8') as file:
        writer = csv.DictWriter(file, fieldnames=["source", "type", "name", "phone", "details", "link", "date"])
        writer.writeheader()
        writer.writerows(all_sellers)
    print(f"DONE! Saved {len(all_sellers)} sellers to {filename}")
else:
    print("No sellers found. Check errors above.")
